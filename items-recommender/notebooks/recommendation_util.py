...
class Formation:
    ...
    def create_matrix_similarity(self):
        ...
        for user in range(self.n_users):
            for item in range(self.n_items):
                ...
                self.matrix_pred[user][item] = sum(user_purchase*top_neighbor_item_scores)/                                                sum(top_neighbor_item_scores)
    ...
    def create_recommendations(self, csv_output=True):
        ...
        self.df_top10 = df_recommend[[c]+list(df_recommend.columns[:r])]             .astype(np.int64).set_index(c).loc[customers[c]]
        self.df_top10['recommendedProducts'] = self.df_top10[list(range(n_recommendations))]             .apply(lambda x: '|'.join(x.fillna('').map(str)), axis=1)
        if csv_output:
